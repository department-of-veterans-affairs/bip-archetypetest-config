...
data "aws_secretsmanager_secret" "consul-token" {
  name = "uat/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "uat/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "uat-bip-archetypetest-container"
...
module "uat_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-uat.fti-stage.*"]
  ...
  tags = merge(
    {
      "Environment" = "STAGE",
      "env"         = "uat"
      "ResTag"      = "UAT BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
