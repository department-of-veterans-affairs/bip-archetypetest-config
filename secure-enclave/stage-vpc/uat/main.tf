...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetestconfig-ivv-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "uat/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "uat/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "uat-bip-archetypetestconfig-container"
...
module "uat_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-uat.fti-stage.*"]
  ...
  tags = merge(
    {
      "Environment" = "STAGE",
      "env"         = "uat"
      "ResTag"      = "UAT BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
