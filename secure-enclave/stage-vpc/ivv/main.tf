...
data "aws_secretsmanager_secret" "consul-token" {
  name = "ivv/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "ivv/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "ivv-bip-archetypetestconfig-container"
...
module "ivv_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-ivv.fti-stage.*"]
  ...
  tags = merge(
    {
      "Environment" = "STAGE",
      "env"         = "ivv"
      "ResTag"      = "IVV BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
