...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-ivv-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "ivv/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "ivv/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "ivv-bip-archetypetest-container"
...
module "ivv_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-ivv.fti-stage.*"]
  ...
  tags = merge(
    {
      "Environment" = "STAGE",
      "env"         = "ivv"
      "ResTag"      = "IVV BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
