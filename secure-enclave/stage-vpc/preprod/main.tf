...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetestconfig-preprod-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "preprod/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "preprod/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_image = "container-registry.stage.bip.va.gov/archetypetest/bip-archetypetestconfig:latest"
     container_name = "preprod-bip-archetypetestconfig-container"
...
module "preprod_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-preprod.fti-stage.*"]
  ...
  tags = merge(
    {
      "Environment" = "STAGE",
      "env"         = "preprod"
      "ResTag"      = "PreProd BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
