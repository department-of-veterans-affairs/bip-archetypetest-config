...
locals {
  ...
  vpc_id = data.terraform_remote_state.fti-svpc.outputs.svpc_id_out

  cluster_id = data.terraform_remote_state.fti-svpc.outputs.svpc_cluster_id_out
  cluster_name = data.terraform_remote_state.fti-svpc.outputs.svpc_cluster_name_out
  listener_arn = data.terraform_remote_state.fti-svpc.outputs.svpc_alb_listener_arn_out

  region = data.terraform_remote_state.fti-global.outputs.region_out

  log_group_name = "/archetypetest/bip-archetypetest/${var.environment}"
...
variable "secure_enclave_container_role_arn" {
  ...
  default = "arn:aws-us-gov:iam::999999999999:role/project/project-preprod-bip-archetypetest-container-role"
}