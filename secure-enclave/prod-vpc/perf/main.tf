...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetestconfig-perf-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "perf/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "perf/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "perf-bip-archetypetestconfig-container"
...
module "perf_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-perf.fti-prod.*"]
  ...
  tags = merge(
    {
      ...
      "env"         = "perf"
      "ResTag"      = "Perf BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
