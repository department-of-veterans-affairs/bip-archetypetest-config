...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-perf-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "perf/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "perf/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "perf-bip-archetypetest-container"
...
module "perf_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-perf.fti-prod.*"]
  ...
  tags = merge(
    {
      ...
      "env"         = "perf"
      "ResTag"      = "Perf BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
