...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-prodtest-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "prodtest/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "prodtest/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_image = "container-registry.prod.bip.va.gov/archetypetest/bip-archetypetest:latest"
     container_name = "prodtest-bip-archetypetest-container"
...
module "prodtest_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-prodtest.fti-prod.*"]
  ...
  tags = merge(
    {
      ...
      "env"         = "prodtest"
      "ResTag"      = "ProdTest BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
