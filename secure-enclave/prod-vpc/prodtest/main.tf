...
terraform {
  backend "s3" {
    ...
    bucket         = "fti-tf-states-prod"
    key            = "terraform.tfstate.bip-archetypetestconfig-prodtest-environment"
    dynamodb_table = "fti-tf-lock-prod"
...
data "terraform_remote_state" "fti-pvpc" {
  backend = "s3"
  config = {
    bucket         = "fti-tf-states-prod"
    key            = "terraform.tfstate.fti-prod-vpc"
    dynamodb_table = "fti-tf-lock-prod"
    region         = "us-gov-west-1"
    encrypt        = true
  }
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "prodtest/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "prodtest/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_image = "container-registry.prod.bip.va.gov/archetypetest/bip-archetypetestconfig:latest"
     container_name = "prodtest-bip-archetypetestconfig-container"
     ...
     environment = [
                        ...
                        },
                        {
                          ...
                          value = "archetypetest/bip-archetypetestconfig"
                        },
                        {
                        ...
                        },
                        {
                          ...
                          value = "consul-fti.prod.bip.va.gov"
                        },
                        {
...
module "prodtest_bip_archetype_test_config_container" {
  ...
  deployment_name                   = "bip-archetypetestconfig"
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-prodtest.fti-prod.*"]
  ...
  tags = merge(
    {
      ...
      "env"         = "prodtest"
      "ResTag"      = "ProdTest BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
