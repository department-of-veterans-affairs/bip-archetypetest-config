...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-prod-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "prod/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "prod/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "prod-bip-archetypetest-container"
...
module "prod_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-prod.fti-prod.*"]
  ...
  tags = merge(
    {
      ...
      "env"         = "prod"
      "ResTag"      = "Prod BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
