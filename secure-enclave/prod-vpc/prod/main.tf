...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetestconfig-prod-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "prod/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "prod/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "prod-bip-archetypetestconfig-container"
...
module "prod_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-prod.fti-prod.*"]
  ...
  tags = merge(
    {
      ...
      "env"         = "prod"
      "ResTag"      = "Prod BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
