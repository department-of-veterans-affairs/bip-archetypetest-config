...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-dev-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "dev/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "dev/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "dev-bip-archetypetest-container"
...
module "dev_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-dev.fti-dev.*"]
  ...
  tags = merge(
    {
      "Environment" = "DEV",
      "env"         = "dev"
      "ResTag"      = "Dev BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
