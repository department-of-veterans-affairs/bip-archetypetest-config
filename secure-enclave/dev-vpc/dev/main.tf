...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetestconfig-dev-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "dev/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "dev/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "dev-bip-archetypetestconfig-container"
...
module "dev_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-dev.fti-dev.*"]
  ...
  tags = merge(
    {
      "Environment" = "DEV",
      "env"         = "dev"
      "ResTag"      = "Dev BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
