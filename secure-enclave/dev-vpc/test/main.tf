...
terraform {
  backend "s3" {
    ...
    bucket         = "fti-tf-states-dev"
    key            = "terraform.tfstate.bip-archetypetestconfig-test-environment"
    dynamodb_table = "fti-tf-lock-dev"
...
data "terraform_remote_state" "fti-dvpc" {
  backend = "s3"
  config = {
    bucket         = "fti-tf-states-dev"
    key            = "terraform.tfstate.fti-dev-vpc"
    dynamodb_table = "fti-tf-lock-dev"
    region         = "us-gov-west-1"
    encrypt        = true
  }
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "test/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "test/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_image = "container-registry.dev.bip.va.gov/archetypetest/bip-archetypetestconfig:latest"
     container_name = "test-bip-archetypetestconfig-container"
     ...
     environment = [
                        ...
                        },
                        {
                          ...
                          value = "consul-fti.dev.bip.va.gov"
                        },
                        {
...
module "test_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-test.fti-dev.*"]
  ...
  tags = merge(
    {
      "Environment" = "DEVTEST",
      "env"         = "test"
      "ResTag"      = "Test BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
