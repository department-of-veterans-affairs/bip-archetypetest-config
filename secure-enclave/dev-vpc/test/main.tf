...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-test-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "test/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "test/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_image = "container-registry.dev.bip.va.gov/archetypetest/bip-archetypetest:latest"
     container_name = "test-bip-archetypetest-container"
...
module "test_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-test.fti-dev.*"]
  ...
  tags = merge(
    {
      "Environment" = "DEVTEST",
      "env"         = "test"
      "ResTag"      = "Test BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
