...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetestconfig-int-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "int/bip-archetypetestconfig/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "int/bip-archetypetestconfig/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "int-bip-archetypetestconfig-container"
...
module "int_bip_archetype_test_config_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetestconfig-int.fti-dev.*"]
  ...
  tags = merge(
    {
      "Environment" = "DEVTEST",
      "env"         = "int"
      "ResTag"      = "Int BIP-ARCHETYPETESTCONFIG Container",
      "Role"        = "Web"
    },
    local.tags
  )
