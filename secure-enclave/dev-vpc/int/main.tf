...
terraform {
  backend "s3" {
    ...
    key            = "terraform.tfstate.bip-archetypetest-int-environment"
...
data "aws_secretsmanager_secret" "consul-token" {
  name = "int/bip-archetypetest/consul-acl-token"
}
data "aws_secretsmanager_secret" "vault-token" {
  name = "int/bip-archetypetest/vault-token"
}
...
module "ecs_task_container_definition" {
     ...
     container_name = "int-bip-archetypetest-container"
...
module "int_bip_archetype_test_container" {
  ...
  vpc_id                            = local.vpc_id
  host_pattern_match_for_listener   = ["*bip-archetypetest-int.fti-dev.*"]
  ...
  tags = merge(
    {
      "Environment" = "DEVTEST",
      "env"         = "int"
      "ResTag"      = "Int BIP-ARCHETYPETEST Container",
      "Role"        = "Web"
    },
    local.tags
  )
